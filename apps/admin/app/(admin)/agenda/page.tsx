    ...
    <section className="page-stack">
      ...
      {branchId && view === 'day' && (
        <section className="calendar-card">
          <div className="calendar-title"><div><p className="eyebrow">Vista día</p><h2>{selectedDate.toLocaleDateString('es-MX',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</h2></div><span>{cabins.length} {cabins.length===1?'cabina':'cabinas'}</span></div>
          {loading ? <div className="fullscreen-inline"><div className="spinner"/><span>Cargando agenda…</span></div> : cabins.length ? (
            <div className="calendar-scroll">
              ...
              <div className="calendar-body" style={{gridTemplateColumns:`82px repeat(${cabins.length}, minmax(190px, 1fr))`}}>
                ...
                {cabins.map((cabin)=><div className="cabin-track" key={cabin.id} style={{height:bodyHeight, backgroundSize:`100% ${SLOT_HEIGHT}px`}}>
