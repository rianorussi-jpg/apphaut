...
export default function ClientesPage(){
 ...
 {loading?<div className="fullscreen-inline"><div className="spinner"/></div>:<div className="card-grid three-cols">{clients.map(c=><button type="button" onClick={()=>{setClientId(c.id);setSuccess('');setError('');}} className="panel-card" key={c.id} style={{textAlign:'left',cursor:'pointer',borderColor:clientId===c.id?'#7e5f50':undefined}}><h3>{c.full_name}</h3><p>{c.phone||'Sin teléfono'}</p><p>{c.email||''}</p><p className="muted">Planes: {plans.filter(p=>p.client_id===c.id).length} · Activos: {plans.filter(p=>p.client_id===c.id&&p.status==='active').length}</p></button>)}</div>}
