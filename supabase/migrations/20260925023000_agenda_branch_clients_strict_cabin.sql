-- HAUT: selector por sucursal, catálogo operativo y reserva estricta de cabina.
-- Ejecutar una sola vez después de las migraciones anteriores.
begin;

-- Asegura que todos los tratamientos activos actuales estén habilitados en todas
-- las sucursales activas y tengan cabinas disponibles si todavía no tenían una
-- configuración específica.
insert into public.treatment_branches (treatment_id, branch_id, is_active, price_override, duration_override_minutes)
select t.id, b.id, true, null, null
from public.treatments t
cross join public.branches b
where t.is_active and b.is_active
on conflict (treatment_id, branch_id) do update set is_active = true;
...
join public.cabins c on c.branch_id = tb.branch_id and c.is_active
where tb.is_active
on conflict do nothing;

-- catalog_details_pending describe una ficha comercial incompleta. El personal
-- administrativo sí puede iniciar un plan; los valores operativos guardados en
-- la tabla siguen siendo válidos hasta que se complete la ficha.
...
begin
  if exists (
    ...
    raise exception 'Este tratamiento todavía tiene datos pendientes y debe ser iniciado por personal de HAUT.';
  end if;
  return new;
end;
$$;

-- Directorio específico de una sucursal para la agenda.
create or replace function public.admin_booking_clients_for_branch(p_branch_id uuid)
...
begin
  ...
  return query
  select p.id,
         coalesce(nullif(pg_catalog.btrim(p.full_name), ''), nullif(pg_catalog.btrim(u.raw_user_meta_data ->> 'full_name'), ''), u.email, 'Cliente')::text,
         coalesce(p.phone, nullif(pg_catalog.btrim(u.raw_user_meta_data ->> 'phone'), ''))::text,
         u.email::text
  from public.profiles p
  left join auth.users u on u.id = p.id
  where not exists (select 1 from public.user_roles ur where ur.user_id = p.id)
    and (
      p.preferred_branch_id = p_branch_id
      or exists (
        select 1 from public.client_treatment_plans pl
        where pl.client_id = p.id
          and pl.default_branch_id = p_branch_id
          and pl.status in ('active','paused')
      )
      or exists (
        select 1
        ...
        where pl.client_id = p.id and a.branch_id = p_branch_id
      )
    )
  order by 2, 1;
end;
...
revoke all on function public.admin_booking_clients_for_branch(uuid) from public, anon;
grant execute on function public.admin_booking_clients_for_branch(uuid) to authenticated;
...
begin
  ...
  if p_preferred_cabin_id is not null then
    if not exists (
      select 1 from public.cabins c
      where c.id = p_preferred_cabin_id
        and c.branch_id = p_branch_id
        and c.is_active
    ) then
      raise exception 'La cabina seleccionada no pertenece a esta sucursal o está desactivada.';
    end if;

    if not exists (
      select 1 from public.treatment_cabins tc
      where tc.treatment_id = p_treatment_id
        and tc.branch_id = p_branch_id
        and tc.cabin_id = p_preferred_cabin_id
    ) then
      raise exception 'Este tratamiento no es compatible con la cabina seleccionada. Corrige la cabina o la compatibilidad del tratamiento antes de crear la cita.';
    end if;

    if exists (
      select 1 from public.schedule_blocks b
      where b.branch_id = p_branch_id
        and b.cabin_id = p_preferred_cabin_id
        and tstzrange(b.starts_at, b.ends_at, '[)') && tstzrange(v_starts_at, v_ends_at, '[)')
    ) then
      raise exception 'La cabina seleccionada está bloqueada en ese horario.';
    end if;

    if exists (
      select 1 from public.appointments a
      where a.cabin_id = p_preferred_cabin_id
        and a.status in ('pending', 'confirmed', 'arrived')
        and tstzrange(a.starts_at, a.ends_at, '[)') && tstzrange(v_starts_at, v_ends_at, '[)')
    ) then
      raise exception 'La cabina seleccionada ya está ocupada en ese horario. Corrige el horario en la agenda antes de continuar.';
    end if;
  end if;
  ...
  for v_cabin in
    ...
    where tc.treatment_id = p_treatment_id
      and tc.branch_id = p_branch_id
      and c.is_active
      and (p_preferred_cabin_id is null or c.id = p_preferred_cabin_id)
    order by
  ...
  loop
    ...
    exception
      when exclusion_violation then
        if p_preferred_cabin_id is not null then
          raise exception 'La cabina seleccionada acaba de ocuparse en ese horario. Recarga la agenda y corrige el conflicto.';
        end if;
        continue;
